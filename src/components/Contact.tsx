  ...
  return (
    <div className={styles.contactWrapper}>
      ...
      <div style={{ textAlign: 'center', paddingTop: '0.5rem' }}>
        <Link href={`mailto:${userinfo.contact.email ? userinfo.contact.email : ''}`}><div>{userinfo.contact.email}</div></Link>
      </div>
      {
        userinfo.contact.phone ?
          <div style={{ textAlign: 'center', paddingTop: '0.2rem', color: currentTheme.tertiary }}>
            <Link href={`tel:${userinfo.contact.countrycode}${userinfo.contact.phone}`}><div>{`${userinfo.contact.countrycode}${userinfo.contact.phone}`}</div></Link>
          </div> : null
      }
          ...
          userinfo.socials.map((social, key) => {
            return (
              <div className={styles.socialIcon} style={iconStyles} key={key}>
                <Link href={social.link}><div><FontAwesomeIcon icon={social.icon} /></div></Link>
              </div>
            )
          }) : null
        }
